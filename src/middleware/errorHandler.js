...
const errorHandler = (err, req, res, next) => {
  ...
  // 1. Erreur de syntaxe JSON dans le corps de la requete
  if (err instanceof SyntaxError && err.status === 400 && 'body' in err) {
    return sendError(
      res,
      {
        code: ErrorCodes.VALIDATION_ERROR,
        message: 'Le corps de la requete contient un format JSON invalide',
        details: {}
      },
      400
    );
  ...
  // 3. Erreur Mongoose : Cle dupliquee (ex: email ou slug existant)
  if (err.code === 11000) {
    ...
    return sendError(
      res,
      {
        ...
        message: `Une ressource avec cette valeur existe deja pour le champ : ${field}`,
        details: { [field]: 'Valeur deja utilisee' }
      },
      409
    );
  }

  // 4. Erreur Mongoose : Validation schema
  if (err.name === 'ValidationError' && err.errors) {
    ...
    return sendError(
      ...
      {
        code: ErrorCodes.VALIDATION_ERROR,
        message: 'Erreur de validation de la base de donnees',
        details
      },
      422
    );
  ...
  // 5. Erreurs JWT (Token expire ou invalide)
  if (err.name === 'JsonWebTokenError' || err.name === 'TokenExpiredError') {
    return sendError(
      res,
      {
        code: ErrorCodes.UNAUTHORIZED,
        message: err.name === 'TokenExpiredError' ? 'Votre session a expire' : 'Token d authentification invalide',
        details: {}
      },
      401
    );
  ...
  // 6. Erreurs metier personnalisees avec statusCode specifique
...
const notFoundHandler = (req, res) => {
  return sendError(
    res,
    {
      ...
      message: `La route demandee ${req.method} ${req.originalUrl} n existe pas`,
      details: {}
    },
    404
  );
